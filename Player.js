...
Player.prototype.update = function() {
	...
	//update the grid value again now that we've moved
	gridVal = grid[this.gridY][this.gridX];
	...
	if (this.wantDirection != this.direction) {
		...
		if (gridVal == 3 || gridVal == (i % 2 ? 2 : 1)) {
			...
			if (!(this.direction%2)) {
				...
				if (((this.x % gridWidth >= gridWidth/2) && (this.xPrev % gridWidth <= gridWidth/2)) ||
				((this.x % gridWidth <= gridWidth/2) && (this.xPrev % gridWidth >= gridWidth/2))) {
					//move to the center of the gridSpace, and change direction
					this.x -= (this.x % gridWidth - gridWidth/2);
					this.direction = this.wantDirection;
				}
			}
			else {
				//want to go from vertical to horizontal
				if (((this.y % gridHeight >= gridHeight/2) && (this.yPrev % gridHeight <= gridHeight/2)) ||
				((this.y % gridHeight <= gridHeight/2) && (this.yPrev % gridHeight >= gridHeight/2))) {
					//move to the center of the gridSpace, and change direction
					this.y -= (this.y % gridHeight - gridHeight/2);
					this.direction = this.wantDirection;
				}
			}
		}
